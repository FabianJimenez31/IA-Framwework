...
- **DP-007**: `pytest` sale con código 5 cuando no recolecta ningún test, y `ci-quality-gate.yml:47` lo ejecuta siempre que exista `pytest.ini` o el directorio `tests/` — ambos se entregan con el boilerplate, este último vacío. El quality gate por tanto falla para todo proyecto nuevo hasta que alguien escriba su primer test. El código 5 se trata ahora como éxito con aviso. La instalación de dependencias también pedía `jq` por pip, que instala un binding de Python y no el CLI que `check-file-size.sh` necesita; el CLI ya viene en el runner.
- **DP-006**: En un pull request, `actions/checkout` deja el HEAD desacoplado, de modo que `git rev-parse --abbrev-ref HEAD` devuelve la cadena literal `HEAD`. `pre-commit.sh` la rechaza por violar la convención de nombres y falla siempre, mientras que `check-prerequisites.sh` pasa **de forma vacua**: nada coincide con `^feature/`, así que nunca llega a validar un solo spec. Se resuelve leyendo `GITHUB_HEAD_REF`.
